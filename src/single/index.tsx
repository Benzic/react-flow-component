...
const SingleFlow: React.FC<FlowProps> = ({ selectedKeys = [], rectConfig, lineCofig, onChange, onEdit }) => {
    ...
    const getSubWidth = useCallback(() => {
        for (let i = 0; i <= action.nodes.length - 1; i++) {
            ...
            for (let j = action.nodes[i].to.length; j--;) {
                let rectIndex = action.getRectIndex(action.nodes[i].to[j])
    ...
    const getSubNode = useCallback((to: any, len: number) => {
        for (let j = to.length; j--;) {
            let rectIndex = action.getRectIndex(to[j])
            if (!action.nodes[rectIndex].to.length) {
                len += 1
            } else {
    ...
    const changeParent = useCallback(() => {
        let rectIndex = action.getRectIndex(action.activeMoveRect)
        if (rectIndex !== null) {
            let parentIndex = action.getRectIndex(action.nodes[rectIndex].parent)
            if (parentIndex !== null) {
    ...
    const onCheckPosition = useCallback((ax: any, ay: any) => {
        if (action.activeMoveRect !== null && action.singleClick) {
            let rectIndex = action.getRectIndex(action.activeMoveRect)
            let parentNode: any = action.findRectInCanvas(ax, ay)
            if (parentNode) {
                let parentIndex = action.getRectIndex(parentNode.key)
    ...
    useEffect(() => {
        ...
        if (selectedKeys.length) {
            action.activeMoveRect = selectedKeys[selectedKeys.length - 1].key
            action.activeDeleRect = selectedKeys[selectedKeys.length - 1].key
            onCheckPosition(selectedKeys[selectedKeys.length - 1].x, selectedKeys[selectedKeys.length - 1].y)
        }
    }, [selectedKeys, onCheckPosition])
